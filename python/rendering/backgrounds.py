...
def lerp_color(
    color_a: tuple[int, int, int], color_b: tuple[int, int, int], amount: float
) -> tuple[int, ...]:
...
def render_background_texture(width: int, height: int) -> Image.Image:
    ...
    for y in range(small_height):
        ...
        for x in range(small_width):
            ...
            glow = 1 - min(
                1,
                ((horizontal_amount - 0.52) ** 2 * 4.0)
                + ((vertical_amount - 0.42) ** 2 * 2.4),
            )
    ...
    for _ in range(random.randint(10, 18)):
        ...
        draw.line(
            (start_x, start_y, end_x, end_y), fill=color, width=random.randint(1, 3)
        )
    ...
    for _ in range(random.randint(5, 9)):
        ...
        draw.ellipse(
            (x, y, x + fog_w, y + fog_h), fill=(255, 255, 235, random.randint(10, 24))
        )
...
def render_background(width: int, height: int, training: bool = False) -> Image.Image:
    if training:
        return random.choice(
            get_background_pool(width, height, BACKGROUND_POOL_SIZE)
        ).copy()
